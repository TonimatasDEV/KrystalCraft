...
public class CuttingFactoryBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new CuttingFactoryBlockEntity(pos, state);
    }
    ...
    protected void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof CuttingFactoryBlockEntity specifiedBlockEntity) {
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.getBlockEntity(pos) instanceof CuttingFactoryBlockEntity specifiedBlockEntity) {
            if (!world.isClient) {
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return validateTicker(type, ModBlockEntities.CUTTING_FACTORY_BLOCK_ENTITY,
